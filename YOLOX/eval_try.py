...
def generate_confusion_matrix(output_data, dataset, save_path="confusion_matrix.png"):
    y_true = []
    y_pred = []
    ...
    for img_id, pred in output_data.items():
        ...
        gt_classes = [ann["category_id"] for ann in imgid_to_anns[img_id]]

        # 如果一张图中预测数量和GT数量不一致，按较小长度对齐（可替换成IoU匹配更精细）
        min_len = min(len(pred_classes), len(gt_classes))
        y_true.extend(gt_classes[:min_len])
        y_pred.extend(pred_classes[:min_len])

    labels = [cat["id"] for cat in coco.cats.values()]
    class_names = [cat["name"] for cat in coco.cats.values()]
